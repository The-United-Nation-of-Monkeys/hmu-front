...
import { UploadSpendingItemsDialog } from '@/components/gov/UploadSpendingItemsDialog'
import { Upload, Receipt } from 'lucide-react'

export function GovGrantDetail() {
  const { grantId } = useParams<{ grantId: string }>()
  const { t } = useTranslation()

  const { data: grant, isLoading } = useQuery({
    queryKey: ['grant', 'government', grantId],
    queryFn: () => grantsApi.getGrant(Number(grantId)),
    enabled: !!grantId,
  })
